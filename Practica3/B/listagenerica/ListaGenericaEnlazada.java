...
public class ListaGenericaEnlazada<T> extends ListaGenerica<T> {
	...
	public ListaGenericaEnlazada<T> invertir(ListaGenericaEnlazada<T> lista, ListaGenericaEnlazada<T> listaInvertida,
			int cont) {
		if (lista.esVacia()) {
			return lista;
		} else {
			if (!lista.fin()) {
				listaInvertida.agregarInicio(lista.elemento(cont));
				lista.proximo();
				cont++;
				invertir(lista, listaInvertida, cont);
